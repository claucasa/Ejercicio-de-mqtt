...
def on_message(mqttc,data,msg):
    ...
    if data['status']==0:
        temp = int(msg.payload) #we are only subscribed in temperature
        ...
        if msg.topic==HUMIDITY:
            ...
            if humidity>data['humidity_threshold']:
                print(f'umbral superado{temp}, subscribiendo a humidity')
                mqttc.subscribe(HUMIDITY) #Esto deber ser lo ultimo
                data['status']=0
    elif TEMP in msg.topic:
        temp = int(msg.payload)
        if temp<=data['temp_threshold']:
            print(f'temperatura{temp} por debajo del umbral,cancelando suscripción')
            data['status']=0
